...
function killServerProcesses() {
    pid="$(pgrep ios-notification-sender-service)"
    if [ "$pid" != "" ]
    then
        echo "killing ios-notification-sender-service"
        kill $pid
    fi
...
while true; do
    ...
    then
        ...
        then
            ...
            cd /swift-project/
            /swift-utils/tools-utils.sh debug ios-notification-sender-service 1024
        else
            # assumes you are running from "normal"/host operating system
            swift build
            if [[ $? == 0 ]]; then 
                # only start server is build was successful (result code 0)
                ./.build/debug/ios-notification-sender-service &
            fi
        fi
