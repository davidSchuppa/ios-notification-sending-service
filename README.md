...
1. `swift build`
1. `.build/debug/ios-notification-sender-service`
...
#### Kubernetes
To deploy your application to your Kubernetes cluster, run `helm install --name myapp .` in the `/chart/ios-notification-sender-service` directory. You need to make sure you change the `repository` variable in your `chart/ios-notification-sender-service/values.yaml` file points to the docker image containing your runnable application.
